...
const QuestionReview: React.FC = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      setError('');
      try {
        // Fetch pending questions for review list
        const pendingRes = await fetch(`${API_BASE_URL}/api/submissions?status=pending`, {
          headers: { Authorization: `Bearer ${jwt}` },
        });
        // Fetch all questions for stats
        const allRes = await fetch(`${API_BASE_URL}/api/submissions`, {
          headers: { Authorization: `Bearer ${jwt}` },
        });
